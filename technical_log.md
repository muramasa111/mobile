...
- `Star` の明るさをデモ用の `brightness` から、実視等級を表す `magnitude` に変更した。
- 赤経を横360度、赤緯を縦180度へ単純投影した概略天球座標を使い、4星座と夏の大三角の相対位置を実際の空に近づけた。
- 描画時は等級が小さいほど星が大きくなるように半径を計算する。
- こと座は、ベガとゼータ星、ゼータ・デルタ・ガンマ・ベータ星による四辺形の形へ星座線を修正した。
## 2026-07-15 01:00 +09:00

### 天球座標の左右関係

- 一般的な北を上にした天球図に合わせ、赤経に相当する横座標を反転した。
- 描画・タップ判定・初期表示位置が同じ座標変換を使うため、見た目と操作位置は一致する。
## 2026-07-15 01:24 +09:00

### やぎ座の星座線

- αをβへつなぐ線と、β・δ・ωによる三角形で表す形に変更した。
## 2026-07-15 01:52 +09:00

### アプリ内天球と方位計

- 方位計の角度をドラッグ量だけから求める方式ではなく、画面中央に対応する天球の横座標から求める方式へ変更した。
- 実世界のコンパスではなく、アプリ内の仮想天球で画面中央が向いている方向を表す。
## 2026-07-15 02:07 +09:00

### 天球座標メーター

- 方位計の `N/E/S/W` を廃止し、赤経を時・分、赤緯を度で示す天球座標メーターへ変更した。
- 外側の半円目盛りは赤経の時間表示で、横ドラッグに応じて変化する。
- 中央の表示は画面中央に対応する赤経と赤緯で、縦ドラッグでは赤緯が変化する。
